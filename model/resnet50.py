...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000,out1=4,out2=65):
        self.inplanes = 64
        ...
        self.fc = nn.Linear(512 * block.expansion, out1)  #因为要预测四个类别，所以输出维度重置成4
        # self.fc = nn.Sequential(nn.Linear(2048, 512),
        #                         nn.ReLU(inplace=True),
        #                         nn.Dropout(p=0.5),
        #                         nn.Linear(512, 4))
        self.fc.apply(weights_init)  #随机初始化

        self.counting = nn.Linear(512 * block.expansion, out2)  #因为要预测65个数，所以输出维度重置成65
    ...
    def forward(self, x, x_new):
        ...
        cls = self.fc(x)  #[B,2048]->[b,4]
        cou = self.counting(x)  ##[B,2048]->[b,65]

        cls = F.softmax(cls) + 1e-4  #归一化
        cou = F.softmax(cou) + 1e-4  #归一化
        ...
        return cls, cou #[resnet50对评级的预测，resnet50对个数的预测]
