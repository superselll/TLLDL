...
def trainval_test(model,train_loader,test_loader, generator, log, epochs, learning_rate, sigma, lam, result_dir, num_label,num_classes, rate):
    ...
    O = generator.matrix.permute(1,0)
    O.requires_grad = False
    start_end_array = torch.from_numpy(generator.startend).to(device).float()
    ...
    log.write(f'startendarary:{start_end_array.cpu().numpy().tolist()}')
    ll = label.detach().to("cpu").numpy()
    number = O.to("cpu").numpy()
    np.savetxt(os.path.join(result_dir, 'O.txt'), number, fmt='%d', delimiter=',')
    np.savetxt(os.path.join(result_dir, 'label.txt'), ll, fmt='%d', delimiter=',')
